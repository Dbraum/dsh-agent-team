...
- The three incremental Session folds keep one cursor per Member instead of one per Session. The context projection and the clock baseline cached their fold state under the Session id, so a rollover left the previous generation's entry — the projection state that had accumulated that generation's checkpoints and continuations — in the map for the life of the process, while only the Member's current Session could ever read it. Both now use the shape the pressure notice already had: a cache keyed by Member holding the Session id the cursor was built for, replaced when that Session changes, so the retained state is bounded by the roster rather than by the number of generations. Cursor ownership is stated once, as `advanceOwnedSessionEventCursor`, and all three call sites share it; the unused `cursorSubjectFor` export it replaces is gone. Folded values are unchanged — a Member whose Session changed folds cold from that Session's own inherited cut, exactly as a first read does.

- Three Host projections stop re-reading a Member Session's whole event log on every step. Context intent, the clock baseline, and the one-shot pressure notice were each a full left fold over `ownEvents()` — once per successful tool result and turn end, once per pre-step, and once per step respectively — so their cost grew with the Session and a long-lived Member got steadily slower to wake. Each now folds through a Session event cursor that keeps the folded value beside the position it consumed: the first read of a log folds it once, and every later read folds only the events that arrived since. On the real 18,983-event session the step path falls from 1.8–2.0 ms to 0.002 ms and stops tracking the log (cold 0.58 → 1.77 ms across 1,898 / 9,491 / 18,983 events while the incremental step stays at 0.002–0.004 ms), with all three folded values byte-identical to the full fold they replace. The cursor trusts the log's append-only contract and verifies what it can: it remembers the sequence and type of the event it stopped on, so a log whose fork prefix moved, that shrank, or that replaced that event re-folds from the start — a fork, resume, or rollover costs a cold fold at worst and never yields a wrong picture. One residual is named in the module rather than papered over: an event replaced in place by another of the same type at the same sequence is invisible to that guard; reaching it requires violating the append-only contract, and a content-sensitive anchor would not close it either, since only the anchor position is re-read while a swap at any earlier folded position would pass the same way.

- A Client surface no longer misses a change that committed while it had no poll. `TeamChangeStream` keeps the last version each scope answered with after that scope's last subscriber leaves, so a surface that returns — a Thread left and re-entered — resumes from that version and the missed commit arrives as a wake instead of being folded into a fresh silent sample; a scope's first subscription in a Client keeps the silent sample, and every version stays inside the cursor domain of the scope that issued it. The Thread page also stops comparing a presence epoch with a durable ledger sequence: roster-and-view coverage is tracked per scope, so a presence transition can no longer swallow a later workspace commit, while one Host event that reaches both scopes still costs a single refresh round.

- A committed Thread read now stores the progress it made instead of the Thread it answered with: the durable record is a receipt — Workspace, Member, Thread ref, optional Task ref, read watermark, Inbox delta — and everything else it once froze beside that delta is re-derived from the projection at every replay. On the 11.7k-operation frozen ledger the read record fell from 5,069 to 894 UTF-8 bytes (−82%; ledger 46.2 → 16.6 MB, −64%), one full record-level replay from 796.9 to 466.7 ms (−41%), and a real-assembly boot from 1,917 to 1,190 ms (−38%). Records written before this release keep the frozen form: one strict union accepts both shapes, a legacy record is normalized and validated through its own full derivation, and loading never rewrites or migrates a stored byte; a record that fits neither shape fails the domain open instead of being guessed at. An identical retry still returns that read's original receipt, but answers with the picture the current projection derives — a read has never promised a frozen answer.

- A private read no longer invalidates any Client. The change cursor is the durable ledger position of the newest commit that changed a shared projection, so a private read — and an audit-only `team/dm-sent` — moves it nowhere; a `presence` scope carries its own process-local wake epoch instead of sharing one counter with every projection scope, so an Agent turning busy or idle no longer wakes the Inbox badge or page. The Client keeps each cursor in the scope it was issued for and re-anchors whenever the Host answers with any different version, so a cursor left over from another scope or an earlier Host lifetime is corrected rather than parking a subscription blind.
- Reading an already-read Thread no longer writes anything. A read whose Inbox delta is empty appends no `team/thread-read` operation, advances no version and schedules no deferred replay, while answering with the same projection-derived picture and the current next-write token. `AgentTeamThreadReadResult.receipt` is absent exactly when the read committed nothing, so a no-op read never presents a fabricated operation receipt, and the render says `Read — no unread updates …` instead of claiming a commit.

- Startup validates the durable ledger once instead of twice: the invariant's mount adopts the record-level replay the ledger constructor already ran over the same records — the same independent scratch-projection replay of every record, never a narrowed one — and stops adopting the moment anything has committed, so the guard is the operations table's single-writer identity (record count, head sequence, head operation id) rather than trust. An unreplayable ledger still fails startup.

- Opening a Thread no longer blocks the Host on the invariant's full ledger replay: the replay is still the same record-level check over the whole durable table, but a burst of commits coalesces into a single run scheduled after the committing call has returned, so a Thread read answers in milliseconds instead of stalling every live Session for O(ledger) time. A divergence stays fatal: the deferred replay logs it, and every later commit re-raises it on a caller-owned frame until a replay comes back clean.

- The 「提到我」 Inbox rows are a queue of full-width 8px-radius hit targets instead of hairline-separated rows: each row carries the shared hover fill and an inset focus ring, the `workspace / #channel` breadcrumb with the `Task #N` 6px chip and an 11px `tabular-nums` time on its first line, and the Thread's 120-character opening line on its own 14px primary line below. The separator hairline is retired with the row fill — it had referenced `--dsw-alias-line-secondary`, a token the shipped theme has never defined, so it never painted and the parity audit failed the tree; the `Task #N` chip keeps its hairline on `--dsw-alias-border-l2`. The page rides the shared conversation seat instead of its own 720px column — the same header band, 880px reading column, responsive gutters, and scrollbar gutter as Channel and Thread — and its header adds a single-line count line, so switching surfaces never shifts the content column and a squeezed seat cannot wrap the header. Its empty/loading/error faces reuse the shared conversation classes.

- The Inbox queue is one global recency order — newest mention first across every Workspace, the ledger sequence breaking ties — instead of Workspace order with each slice in the Host's order, which is the Host's truncation policy rather than a display order. Row times name the two days a queue reader reasons about (「今天 HH:mm」/「昨天 HH:mm」 by the viewer's local calendar day), fall back to the Message form for anything older, and carry the precise local `YYYY-MM-DD HH:mm` on the element's `title`.

- The Inbox count badge is an 18px capsule on both rails: `box-sizing: border-box` keeps a single digit a circle instead of padding it out to an oval, `border-radius: 999px` pairs `corner-shape: round` so the platform superellipse cannot square the capsule ends off, and the narrow rail's badge sits inside its 36px control box because that rail clips its own overflow. The wide card now carries the count in its accessible name; the visual number stays `aria-hidden`.

- Every Team capsule and circle pairs `corner-shape: round` with its full-round radius, matching the shipped platform rule (`ui-theme/src/styles/corner-shape.css`) that shipped keeps at 100% coverage; the parity audit gained a rule that fails an unpaired full-round block, plus geometry pins for the Inbox card, the badge, the Inbox queue row, and its Task chip.

- The `@deepseek-ai/dsh-*` peers narrow from `>=0.1.5-rc.1 <0.2.0` to `>=0.1.5-rc.1 <0.1.6`, so the manifest declares exactly the certified DSH line instead of also admitting every later stable `0.1.x` release, none of which was certified. A newer line therefore falls outside the declared range — and reports as a peer conflict — rather than installing under a compatibility claim this repository has not verified. The routed `@deepseek-ai/dsh-storage-sqlite` dependency keeps the wider `>=0.1.5-rc.1 <0.2.0`: it is an ordinary dependency, not a peer, so it is resolved with the framework line instead of pinning one certified cut.

- The Human Team gains a 「提到我」 Inbox: the Host's Inbox Remote gains a direct-only slice that returns only Threads with an unread mention (`directOnly`), carries each row's rendering preview (Channel display name, optional Task number, the Thread's opening line capped at the same 120 characters the Thread page applies to Task titles), and collapses `totalUnreadCount` to the direct total so a badge can never bypass follow unread. The Client renders the merged cross-Workspace queue — a sidebar card on the wide rail, the first icon on the narrow rail (both carrying the direct badge), and a fourth conversation face whose rows open the row's Workspace and Thread; opening the Inbox page acknowledges nothing, and only the durable Thread read of an opened row clears its marker, row, and badge through the existing auto-ack path. The Client merges the per-Workspace calls itself; there is no home-level Inbox ledger or Remote.

- Thread messages follow a conclusion-first contract with one mention rule: mentioning the Human is how the Human is notified, so the Human is mentioned exactly when they must know or decide — with a one-to-three-sentence opening and a `Decision needed: X (default: Y)` line when a decision is owed (a scanning convention, never an Inbox key) — while mechanical detail (`file:line`, commands, hashes, probe output) follows below and detail a peer Member needs is moved, never dropped; prose follows the Human's language and identifiers, paths, commands, and refs stay verbatim. The contract lives in the `team-member` persona, is restated on the `team_message` body parameter, and is documented in `docs/team-collaboration.md`.
...
- The Channel 「作为任务」 control reads as the mode it is instead of an outlined text button wedged between icon circles: it sits beside the attach control as a labeled toggle pill in the base UI's mode-control language, and only drops its word — never its accessible name, `title`, or `aria-pressed` state — once the composer card is narrower than 460px, the same cut the shipped permission chip uses. The parity audit gained a mode-control rule (visible label, container-only label cut, pressed state, pill geometry) so a later refactor cannot quietly reduce the mode to a bare glyph.
