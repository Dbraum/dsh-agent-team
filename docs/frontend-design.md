...
| Count badge | 18px capsule: `border-radius: 999px` with `box-sizing: border-box` (one digit stays a circle instead of padding out to an oval), `--dsw-alias-state-business-primary` fill and `--dsw-alias-label-primary-foreground` text, hidden at zero, `99+` past it, and the number rides the control's accessible name (`aria-label`), never the visual badge alone. | `Tag.module.css` (read-only capsule language) |
...
| Inbox row time | 11px/18px, tertiary, `tabular-nums`; 「今天 HH:mm」/「昨天 HH:mm」 by the viewer's local calendar day, the Message form for anything older, and the precise local `YYYY-MM-DD HH:mm` on the element's `title` |
...
Mention chips are rendered only for handles in the Message's resolved mention list, case-insensitively and with an optional `@`. Human literal, Agent plain prose, and rich Markdown use their corresponding segmentation path; absent names become a trailing fallback row without duplication.
...
The 「提到我」 entry is one card on the wide rail above the Workspaces section and the rail's first icon on the narrow rail; both carry the same cross-Workspace direct badge (sum of each visible Workspace's `directOnly` Inbox total), hidden at zero and reading `99+` past it. The visual badge is `aria-hidden`; the count reaches assistive tech through the control's `aria-label` on both rails. On the narrow rail the badge sits inside the 36px icon box — the rail region clips its own overflow, so a badge hung past the button edge loses its right corner. While the Inbox page stands, the card/icon carries `aria-current="page"`.
...
The page rides the shared conversation seat — the same header band, 880px centered reading column, `clamp(18px, 3vw, 36px)` gutters, and stable scrollbar gutter Channel and Thread use — so switching surfaces never shifts the content column. Its header carries the page h1 plus one count line, kept to a single ellipsized line so a squeezed seat cannot wrap the header. It merges one direct-only Inbox call per visible Workspace into a single recency order — newest mention first across every Workspace, the ledger sequence breaking ties — because a queue is read by recency rather than by Workspace (a Workspace slice arrives in the Host's own order, which is the Host's truncation policy, not a display order) — and never acknowledges anything: opening the page is not a read, and only the durable Thread read of an opened row clears its marker and the badge. Each row is one full-width 8px-radius queue row in the shipped two-line result-row shape — 8px horizontal inset, shared `--dsw-alias-interactive-bg-hover` fill, inset focus ring — carrying the Human-fixed information order: (1) a one-line breadcrumb `workspace / #channel`, plus `Task #N` when taskful, (2) the Thread's opening line capped at 120 characters straight from the Host's `previewText`, on its own 14px/22px primary line spanning the full row, and (3) the latest mention instant right-aligned on the breadcrumb line as an 11px tertiary `tabular-nums` label that names the two days a queue reader reasons about — 「今天 HH:mm」/「昨天 HH:mm」 by the viewer's local calendar day — and falls back to the Message form (`MM-DD HH:mm` this year, full date across years) for anything older, with the precise local `YYYY-MM-DD HH:mm` on the element's `title`. Clicking a row selects the row's Workspace and opens its Thread. The empty state speaks the shared empty-state language — the same 13px `strong` title and 12px hint pair Channel and Thread use — reading 「还没有人提到你」 with the hint 「需要你知道或做决定时，成员会提到你」; loading, error, and retry reuse the shared conversation classes, and a failed background refresh keeps the rows and reports through `role="alert"` in `--dsw-alias-state-error-primary`. While the page is open it subscribes once without a change scope and refetches the list on every wake; closed, the fetches stop. The badge subscribes the same way for its totals and re-pulls only totals (`limit: 1`), never lists. The badge additionally refreshes from every completed durable Thread read: the Host's changes stream deliberately never wakes on reads (a read changes no shared projection), but the read consumes this reader's mention markers.
