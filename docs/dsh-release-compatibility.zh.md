...
当前 Team bundle 的已认证基线是 DSH `0.1.6-alpha.1`；该切点的记录见本节最后一个小节，紧随其后的几段保留产生上一条基线的 `0.1.5` 历史。DSH peers 正好声明这条已认证线，现为 `>=0.1.6-alpha.1 <0.1.7`，因此本仓库尚未验证的线会落在声明区间之外，而不是在未经核实的兼容声明下被装上。经路由的 `dsh-storage-sqlite` 依赖有意保留更宽的上界——它是普通 dependency 而非 peer——但其下界随切点一起移动：下界若仍停留在上一条线，就会在已认证的 `0.1.6` host 旁边解析出第二套更旧的 DSH storage 代际。
...
DSH `0.1.6-alpha.1` 已认证，并推动基线前移。它落在旧 `>=0.1.5-rc.1 <0.1.6` peers 之外——比较符只在自身 base tuple 上开放预发布，因此 `<0.1.6` 既不准入 `0.1.6-alpha.1`，也不准入 `0.1.6` 本身——所以全部 `@deepseek-ai/dsh-*` peers 整体移动到 `>=0.1.6-alpha.1 <0.1.7`。`dsh-storage-sqlite` 依赖在同一切点移动到 `>=0.1.6-alpha.1 <0.2.0`：它是经路由的 ledger backend，其自身 peers 现在要求 `@deepseek-ai/dsh-storage@^0.1.6-alpha.1`，把下界留在 `0.1.5-rc.1` 会在 `0.1.6` host 旁解析出 `0.1.5` 包。`@deepseek-ai/cordis` 从 `4.0.1` 升到 `4.0.2`，现有 `^4.0.1` peer 已准入。
